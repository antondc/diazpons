...
#[derive(Template)]
#[template(path = "views/author/author.html")]
pub struct AuthorTemplate {
  author_with_books: AuthorWithBooks,
  language: Language,
  twitter_svg: String,
  facebook_svg: String,
...
pub async fn author_view(view_data: ViewData<AuthorWithBooks>) -> AuthorTemplate {
  ...
  let template = AuthorTemplate {
    author_with_books: view_data.data,
