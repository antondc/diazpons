...
pub trait IAuthorsGetDataUseCase {
  ...
  async fn execute(&self, slug: Option<String>) -> Result<DataWithLanguage<Vec<AuthorWithBooks>>>;
}
...
impl<K: ILanguageGetOneOrDefaultUseCase> IAuthorsGetDataUseCase for AuthorsGetDataUseCase<K> {
  ...
  async fn execute(&self, slug: Option<String>) -> Result<DataWithLanguage<Vec<AuthorWithBooks>>> {
    ...
    let books_data: DataWithLanguage<Vec<AuthorWithBooks>> = DataWithLanguage {
